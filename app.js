(() => {
  ...
  function normalize(source){
    ...
    const records=(items,prefix)=>Array.isArray(items)?items.map(r=>({...r,id:String(r.id||uid(prefix)),vehicleId:validVehicle(vehicleIdOf(r)),currency:currencyCode(r.currency),...(prefix==='m'?{types:(r.types||[]).map(fixedCode)}:{}),...(prefix==='e'?{category:fixedCode(r.category)}:{}),...(prefix==='t'?{type:fixedCode(r.type)}:{})})):[];
  ...
  function totalsFor(vehicleId){
    ...
    data.trips.filter(r=>r.vehicleId===vehicleId).forEach(r=>add(r.cost,r.currency));
    return totals;
  }
  ...
  function renderVehicle(){
    ...
    const tripKm=trips.reduce((s,t)=>s+Math.max(0,num(t.endOdometer)-num(t.startOdometer)),0),totals=totalsFor(v.id);
  ...
  function renderReport(){
    ...
    const tripKm=trips.reduce((sum,r)=>sum+Math.max(0,num(r.endOdometer)-num(r.startOdometer)),0),generated=new Intl.DateTimeFormat(locale(),{dateStyle:'long',timeStyle:'short'}).format(new Date()),image=v.image||v.imageUrl||'';
    ...
    if(included('trips'))html+='<section class="report-section"><h2>سجل الرحلات</h2>'+reportTable(['التاريخ','من','إلى','النوع','عداد البداية','عداد النهاية','المسافة','التكلفة'],trips.map(r=>'<tr><td>'+date(r.date)+'</td><td>'+esc(r.start)+'</td><td>'+esc(r.destination)+'</td><td>'+esc(fixedLabel(r.type))+'</td><td>'+metric(r.startOdometer)+'</td><td>'+metric(r.endOdometer)+'</td><td>'+metric(Math.max(0,num(r.endOdometer)-num(r.startOdometer)),'كم')+'</td><td>'+moneyMetric(r.cost,r.currency)+'</td></tr>'))+'</section>';
  ...
  function openForm(type,recordId=null,vehicleOverride=null){
    ...
    if(type==='trip'){const r=data.trips.find(x=>x.id===editingId)||{date:today(),start:'',destination:'',startOdometer:v.odometer,endOdometer:'',type:'local',cost:0,currency:'KWD',notes:''};title=editingId?'تعديل الرحلة':'إضافة رحلة';html=field('نقطة الانطلاق','start','text',r.start)+field('الوجهة','destination','text',r.destination)+field('تاريخ الرحلة','date','date',r.date)+select('نوع الرحلة','type',['local','travel','offroad'].map(value=>({value,text:fixedLabel(value)})),r.type)+field('عداد البداية','startOdometer','number',r.startOdometer,{min:0})+field('عداد النهاية','endOdometer','number',r.endOdometer,{min:0})+'<p class="form-hint" id="distance-preview"></p>'+field('تكلفة الرحلة','cost','number',r.cost,{min:0,step:'.001'})+currencySelect(r.currency)+area('ملاحظات الرحلة','notes',r.notes)}
    ...
    if(type==='trip'){const update=()=>{const s=num($('[name=startOdometer]').value),e=num($('[name=endOdometer]').value);$('#distance-preview').textContent=e>=s&&e?'المسافة المحسوبة: '+fmt(e-s)+' كم':'أدخل عداد النهاية لحساب المسافة';applyLanguage($('#distance-preview'))};$$('[name=startOdometer],[name=endOdometer]').forEach(x=>x.addEventListener('input',update));update()}applyLanguage($('#app-dialog'));setTimeout(()=>$('input,select,textarea',$('#app-form'))?.focus(),0)
  }
  ...
  function submit(e){
    ...
    if(activeForm==='trip'){if(num(x.endOdometer)<num(x.startOdometer))return toast('عداد النهاية يجب أن يكون أكبر من عداد البداية');upsert('trips',{id:editingId||uid('t'),vehicleId:v.id,date:x.date,start:x.start.trim(),destination:x.destination.trim(),startOdometer:num(x.startOdometer),endOdometer:num(x.endOdometer),type:x.type,cost:num(x.cost),currency:currencyCode(x.currency),notes:x.notes.trim()})}
