(() => {
  ...
  function totalsFor(vehicleId){
    ...
    data.trips.filter(r=>r.vehicleId===vehicleId).forEach(r=>Object.entries(tripTotals(r)).forEach(([currency,value])=>add(value,currency)));
    return totals;
  }
  ...
  function renderReport(){
    ...
    if(included('trips')){const finalValue=convertedTotals(tripReportTotals,reportFinalCurrency);html+='<section class="report-section"><h2>سجل الرحلات</h2>'+reportTable(['التاريخ','من','إلى','النوع','عداد البداية','مصروفات الرحلة','إجمالي المسافة','الإجمالي حسب العملة'],trips.map(r=>'<tr><td>'+date(r.date)+'</td><td>'+esc(r.start)+'</td><td>'+esc(r.destination)+'</td><td>'+esc(fixedLabel(r.type))+'</td><td>'+metric(r.startOdometer)+'</td><td class="report-notes">'+(tripExpenses(r).map(item=>esc(tripExpenseLabel(item.type)+(item.type==='maintenance'&&item.maintenanceType?' · '+item.maintenanceType:''))+' — '+moneyMetric(item.cost,tripExpenseCurrency(item,r))).join('<br>')||'—')+'</td><td>'+(tripDistance(r)===null?'—':metric(tripDistance(r),'كم'))+'</td><td>'+tripTotalsHTML(tripTotals(r),'—')+'</td></tr>'))+'</section><section class="report-section report-final-total"><h2>الإجمالي النهائي</h2><div class="report-totals">'+totalsMarkup(tripReportTotals)+'</div><label><span>عملة الإجمالي النهائي</span><select id="report-final-currency">'+currencyOptions(reportFinalCurrency)+'</select></label><strong id="report-final-value">'+(finalValue===null?'—':moneyMetric(finalValue,reportFinalCurrency))+'</strong><small>'+(exchangeError?'تعذر تحميل أسعار الصرف':!exchangeRates?'جارٍ تحميل سعر الصرف…':'تم التحويل باستخدام سعر الصرف الحالي')+'</small></section>'}
