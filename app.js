(() => {
  ...
  function setupTripForm(){
    const form=$('#app-form'),rows=$('#trip-expense-rows',form),currency=$('[name=currency]',form),total=$('[name=totalCost]',form),vehicle=$('[name=vehicleId]',form),startOdometer=$('[name=startOdometer]',form);
    ...
    rows.addEventListener('input',updateTotal);rows.addEventListener('change',e=>{if(e.target.matches('[name=tripExpenseType]'))updateMaintenance(e.target.closest('.trip-expense-row'));updateTotal()});currency.addEventListener('change',updateTotal);if(!editingId)vehicle.addEventListener('change',()=>{const selected=data.vehicles.find(item=>item.id===vehicle.value);if(selected)startOdometer.value=selected.odometer});$$('.trip-expense-row',rows).forEach(updateMaintenance);updateTotal();
  }
  ...
  function submit(e){
    ...
    if(activeForm==='trip'){const tripVehicle=data.vehicles.find(item=>item.id===String(x.vehicleId));if(!tripVehicle)return toast('اختر مركبة صالحة للرحلة');const expenses=$$('.trip-expense-row',e.currentTarget).map(row=>{const type=$('[name=tripExpenseType]',row).value;return{id:row.dataset.expenseId||uid('te'),type,cost:num($('[name=tripExpenseCost]',row).value),maintenanceType:type==='maintenance'?$('[name=tripMaintenanceType]',row).value.trim():''}}),distanceValue=String(x.distanceKm??'').trim(),totalCost=expenses.reduce((sum,item)=>sum+item.cost,0);upsert('trips',{id:editingId||uid('t'),vehicleId:tripVehicle.id,date:x.date,start:x.start.trim(),destination:x.destination.trim(),startOdometer:num(x.startOdometer),type:x.type,expenses,totalCost,cost:totalCost,currency:currencyCode(x.currency),notes:x.notes.trim(),distanceKm:distanceValue===''?null:num(distanceValue)})}
